...
- Start with `start.md`. For SQL Server connection details use `SQL-SERVER-CONNECTION.md`; for script layout use `SCRIPT-STRUCTURE.md`; for module source sync use `MODULE-SOURCE-SYNC.md`; for Maven library-first builds use `MAVEN-LIBRARY-FIRST-BUILD.md`; for log viewer details use `LOG-VIEWER.md`; for NiFi file upload use `NIFI-FILE-TO-KAFKA.md`; for saved log behavior use `LOG-PERSISTENCE.md`; for authenticated proxy setup use `PROXY-CONFIG.md`; for port changes use `PORT-CONFIG.md`; for REST/SOAP/JKS examples use `REST-SOAP-SECURITY-EXAMPLES.md`; for adding a new app use `ADD-NEW-APP.md`.
